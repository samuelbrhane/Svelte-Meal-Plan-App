<script>
  import { onMount, afterUpdate } from "svelte";
  import Chart from "chart.js/auto";

  export let totalDinnerProtein;
  export let totalDinnerFats;
  export let totalDinnerCarbs;

  let chartInstance;

  const createChart = () => {
    const dinnerCtx = document.getElementById("dinnerChart");
    chartInstance = new Chart(dinnerCtx, nutrientData);
  };

  const updateChart = () => {
    chartInstance.data.datasets[0].data[0] = totalDinnerProtein;
    chartInstance.data.datasets[0].data[1] = totalDinnerFats;
    chartInstance.data.datasets[0].data[2] = totalDinnerCarbs;
    chartInstance.update();
  };

  onMount(() => {
    createChart();
  });

  afterUpdate(() => {
    updateChart();
  });

  const nutrientData = {
    type: "polarArea",
    data: {
      datasets: [
        {
          data: [totalDinnerProtein, totalDinnerFats, totalDinnerCarbs],
          backgroundColor: ["#32aac898", "#c832a798", "#85a33498"],
        },
      ],
      labels: ["Protein", "Fats", "Carbs"],
    },
    options: {
      responsive: true,
      scales: {
        r: {
          pointLabels: {
            display: true,
            centerPointLabels: true,
            font: {
              size: 18,
            },
          },
        },
      },
      plugins: {
        legend: {
          position: "top",
        },
        title: {
          display: true,
          text: "Dinner Nutrients Chart",
          color: "#a3a234",
          font: {
            size: 18,
          },
        },
      },
    },
  };
</script>

<div>
  <canvas id="dinnerChart" class="max-w-[300px] max-h-[300px]" />
</div>
