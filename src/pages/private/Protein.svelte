  ...
  $: {
    ...
    days.forEach((day) => {
      ...
      if (!findDay) {
        ...
        // add the total protein for that day
        let currentDateProtein = 0;

        findDay.breakfast.forEach((data) => {
          currentDateProtein += data.nutrients[2].total / 2.8;
        });
  ...
  <div class="lg:ml-[250px]">
    <div class="px-4 md:px-12 py-4 lg:px-20">
      ...
      <div class="flex justify-between mb-6">
        <Title title="Protein" subtitle="Chart of weekly protein intake" />
        <div>
          <!-- data picker  -->
      ...
      <!-- protein chart -->
      <NutrientsLineChart {days} {labelName} data={protein} />
    </div>
  </div>
</PrivateLayout>
